...
def start_btn():
    ...
    if  sorting_algorithm.get() == 'Sorting Algorithm':
        messagebox.showwarning("Warning", "Please select a sorting algorithm.")

    if sorting_algorithm.get() == 'Bubble Sort':
        sorting_alg.bubble_sort(dataset, draw_data, animation_speed.get())
...
graph = tkinter.ttk.Combobox(sidebar_fr, values=['Bar'], textvariable=graph_type)
graph.grid(row=2, column=0, padx=5, pady=5)
graph.current(0)

# Disable the Combobox so the user can't select anything
graph.configure(state='disabled')
